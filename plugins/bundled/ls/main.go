...
// execRequest is the stado_exec JSON payload (EP-0038 §B).
type execRequest struct {
	Argv []string `json:"argv"`
}

// execResult is the stado_exec JSON response.
...
func stadoToolLs(argsPtr, argsLen, resultPtr, resultCap int32) int32 {
	...
	argv := []string{"ls", "-l", "--time-style=long-iso", a.Path}
	if a.Hidden {
		argv = []string{"ls", "-la", "--time-style=long-iso", a.Path}
	}
	out, err := exec(argv...)
	if err != nil {
		return writeJSON(resultPtr, resultCap, errResult{Error: "exec: " + err.Error()})
	}
...
// exec runs argv via stado_exec and returns combined stdout.
func exec(argv ...string) (string, error) {
	req := execRequest{Argv: argv}
	...
	if n < 0 {
		return "", fmt.Errorf("stado_exec: %s", string(scratch[:-n]))
	}
	// stado_exec returns JSON {stdout, exit_code, error?}
	var res execResult
	if err := json.Unmarshal(scratch[:n], &res); err != nil {
		// Older host (stado_exec_bash) returns raw stdout — accept both.
		return string(scratch[:n]), nil
	}
	if res.Error != "" {
		return "", fmt.Errorf("exec: %s", res.Error)
	}
