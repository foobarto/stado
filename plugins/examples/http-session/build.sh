...
# Run from plugins/examples/http-session/.
#
# Prerequisites:
...
#   - http-session-demo.seed in the current directory
#     (generate once with `stado plugin gen-key http-session-demo.seed`)
#
...
if [[ ! -f http-session-demo.seed ]]; then
  echo "http-session-demo.seed not found. Generate it with:" >&2
  echo "  $STADO plugin gen-key http-session-demo.seed" >&2
  exit 1
fi
...
"$STADO" plugin sign plugin.manifest.json --key http-session-demo.seed
