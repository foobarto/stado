...
| `--headless` | Serve the JSON-RPC 2.0 daemon over stdio instead of a one-shot prompt (was `stado headless`). One-shot run flags are rejected rather than ignored; only `--persona` supplies a daemon default. Configure sessions through config and the `session.*` RPC methods. See [headless.md](headless.md). |
...
- **Headless daemon mode is not a one-shot run.** `stado run --headless`
  ...
  JSON-RPC session. Use config plus `session.*` requests; `--persona` is the
  supported daemon default.
