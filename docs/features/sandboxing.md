...
to keep them. Git-over-SSH therefore needs credentials provisioned outside
Stado or an explicitly unsandboxed operator workflow. Short-lived, narrowly
scoped SSH credential provisioning is intentionally a separate system concern
rather than a Stado sub-agent role.
