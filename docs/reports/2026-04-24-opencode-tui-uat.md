...
| Status modal and LSP state | Partial | `/status` and `Ctrl+X S` show runtime health, provider credential health, LSP readiness, action hints, and trace IDs; deeper live provider/plugin/MCP health remains future work. |
