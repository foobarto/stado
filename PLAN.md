...
| D  | ✅ Phase 11.3 — TUI `/compact` + `stateCompactionPending` state + y/n/e confirmation + inline summary editor (`e` key → `stateCompactionEditing`, Enter commits, Esc/'n' cancels, user's in-flight draft preserved) + `internal/compact` package (summarisation prompt + async Summarise) + advisory `stado session compact` CLI stub + **dual-ref compaction commit** (`Session.CommitCompaction`: tree ref keeps its parent's tree hash so `git checkout tree~1 -- …` restores pre-compaction state, trace ref gets a parallel empty-tree marker; both carry the same summary body + turn-range trailers) + **compaction-marker metadata** (11.3.6) via `Sidecar.ListCompactions` — `stado session show <id>` now renders a `compactions` timeline with commit SHA, turn range, timestamp, and author for every compaction event on the tree ref. Full test coverage. | 11 |
...
| F  | ✅ Phase 11.5 — shipped: `session fork <id> --at <turns/N\|sha>` scripted path, standalone `session tree <id>` cobra subcommand with its own tea.Program (navigate + press `f` to fork), `Sidecar.ListTurnRefs` helper, scripted + interactive integration tests, plus teatest-backed PTY harness (`session_tree_pty_test.go`) that drives the real tea.Program through an emulated terminal: navigate → wait for re-render → fork → assert child tree ref round-trips, plus a q-exits-clean test to catch key-dispatch regressions. | 11 |
...
| I  | ⏸️ Phase 3.6 — Windows v1 shipped: `runner_windows.go` WinWarnRunner emits one-time warning + runs unsandboxed. Wine-based smoke verified on Fedora via Bottles' bundled wine 11.0: `stado.exe version`, `plugin digest` (byte-identical sha256 cross-OS), `doctor` (correctly reports `Sandbox runner: windows-passthrough` + Windows-style state dir), and `run --prompt` with the provider-fallback path round-trip through LM Studio on localhost. v2 (job objects + restricted tokens) **deferred** — needs a real Windows dev environment to implement and verify; re-open when someone on the team picks up Windows support. | 3 |
...
| K2 | ✅ Phase 7.1b — session/LLM plugin capabilities: manifest parses `session:observe` / `session:read` / `session:fork` / `llm:invoke[:<budget>]`; `stado_session_read` / `stado_session_next_event` / `stado_session_fork` / `stado_llm_invoke` host imports registered; `SessionBridge` interface + `SessionBridgeImpl` reference implementation wire real `stadogit.Session` + `agent.Provider`; polling variant (`stado_session_next_event`) chosen over the spec's callback-based `stado_session_observe` (wasm-native); TUI `/plugin:...` path constructs a bridge from live session state; `Plugin:` audit trailer on plugin-triggered trace commits (`llm_invoke`, `plugin_fork`); `Model.pluginForkAt` closure drives the real fork primitive, writes a seed marker to the child's trace ref, and dispatches `pluginForkMsg` for the TUI to render a user-visible "plugin X forked session → Y" block (DESIGN invariant 4). Persistent lifecycle shipped: plugins that export `stado_plugin_tick` load once per TUI boot (via `[plugins].background = [...]`) and tick on every turn boundary with a `turn_complete` event queued onto their SessionBridge. Validating examples: `examples/plugins/auto-compact/` exercises all four capabilities end-to-end (session:read → llm:invoke → session:fork) both via `/plugin:` one-shot and as a background plugin; `examples/plugins/session-recorder/` exercises a different capability mix (session:read + fs:read + fs:write + `stado_plugin_tick`) proving the ABI generalises beyond compaction-shaped plugins. Headless parity: `plugin.list` / `plugin.run` RPC methods + background-plugin loading + turn-boundary tick in `internal/headless`; plugin-driven forks emit `session.update { kind: "plugin_fork", plugin, reason }` (per DESIGN invariant 4) with extra `child` / `at_turn_ref` / `childWorktree` metadata so JSON-RPC clients can render or resume without a follow-up call. 43 tests. | 7 |
