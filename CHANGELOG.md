...
- **Sandbox documentation corrected.** Replace the obsolete opt-in-wrapper
  ...
  across TUI, run, headless, ACP, and MCP surfaces, including the honest
  enforcement-availability and defense-in-depth fallbacks. The legacy
  `[sandbox]` wrapper remains an optional additional outer process wrapper; it
  is not the primary tool-execution boundary.
...
- `plugin trust` error directs operators to acquire the author key
  out of band, pin it with `plugin trust`, then retry verification.
