...
func TestRunPluginToolAsyncRejectsOversizedArgsBeforeRuntime(t *testing.T) {
	cmd := runPluginToolAsync(nil, "", nil, plugins.RuntimeIdentity{}, plugins.ToolDef{Name: "compact"},
		strings.Repeat("x", toolinput.MaxBytes+1), "test-plugin", nil, nil, nil, nil, nil, nil)
