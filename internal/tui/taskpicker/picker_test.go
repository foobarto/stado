...
func TestPickerCreateCommand(t *testing.T) {
	...
	cmd, handled := m.Update(tea.KeyPressMsg{Code: 'n', Mod: tea.ModCtrl})
	if !handled || cmd.Type != CommandNone {
		t.Fatalf("ctrl+n handled=%v cmd=%+v", handled, cmd)
	}
	for _, r := range "Ship tasks" {
		cmd, _ = m.Update(tea.KeyPressMsg{Text: string(r)})
	}
...
func TestPickerDetailAndDeleteCommand(t *testing.T) {
	...
	cmd, _ = m.Update(tea.KeyPressMsg{Code: 'd', Mod: tea.ModCtrl})
	if cmd.Type != CommandNone {
		t.Fatalf("delete prompt cmd = %+v", cmd)
	}
	cmd, _ = m.Update(tea.KeyPressMsg{Text: "y"})
...
func TestPickerCapsFormTitleAndBody(t *testing.T) {
	...
	m.Update(tea.KeyPressMsg{Text: strings.Repeat("x", tasks.MaxTitleBytes+128)})
	...
	m.Update(tea.KeyPressMsg{Code: tea.KeyDown})
	m.Update(tea.KeyPressMsg{Text: strings.Repeat("x", tasks.MaxBodyBytes+128)})
