// Package filepicker renders an inline file-path popover triggered by
// typing `@` in the main TUI input. Mirrors the patterns other
// coding-agent TUIs (opencode, pi) use for @-mentions: fuzzy match the
// repo's tracked files, Tab/Enter inserts the selected path replacing
// the `@query` fragment. The actual file contents get attached on the
// model's next turn via the read tool — this picker only handles the
// path-discovery surface, not file loading.
package filepicker
...
// Open shows the picker rooted at cwd and resets Query/Cursor. Scans
// the directory tree synchronously; callers concerned about
// responsiveness on huge repos should cache Open in a tea.Cmd.
...
func (m *Model) refresh() {
	...
	if q := strings.TrimSpace(m.Query); q == "" {
		if len(paths) > maxVisibleMatches {
			m.Matches = paths[:maxVisibleMatches]
		} else {
			m.Matches = paths
		}
		return
	}
	found := fuzzy.Find(m.Query, paths)
	out := make([]string, 0, maxVisibleMatches)
	for i, f := range found {
		if i >= maxVisibleMatches {
			break
		}
		out = append(out, paths[f.Index])
	}
	m.Matches = out
}
