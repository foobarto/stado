...
func (a *FleetBridgeAdapter) AgentSpawn(ctx context.Context, req pluginRuntime.AgentSpawnRequest) (pluginRuntime.AgentSpawnResult, error) {
	...
	if req.Execution == "retained" {
		return a.spawnRetained(ctx, req, opts)
	}
...
func (a *FleetBridgeAdapter) spawnRetained(ctx context.Context, req pluginRuntime.AgentSpawnRequest, opts SpawnOptions) (pluginRuntime.AgentSpawnResult, error) {
	...
	h, err := a.Retained.SpawnRetained(ctx, orchestration.LaunchRequest{AccountID: a.RetainedAccountID, Budget: budget, Principal: a.Principal, Actor: a.ParentSessionID, IdempotencyKey: idem, RestartPolicy: restartPolicy, Launcher: spawnerLauncher{spawner: a.Spawner, request: request, mailbox: a.Retained.Mailbox, principal: a.Principal}, Admission: retained.Request{ParentSessionID: a.ParentSessionID, ChildSessionID: childID, Purpose: request.Role, Fork: fork, CeilingDigest: hex.EncodeToString(ceiling[:]), Model: req.Model, ToolProfile: req.ToolProfile, Principal: a.Principal, Actor: a.ParentSessionID, IdempotencyKey: idem + ":admission"}})
	if err != nil {
		return pluginRuntime.AgentSpawnResult{}, err
	}
