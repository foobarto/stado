...
func TestSessionNewUsesIndependentBrokerHandles(t *testing.T) {
	...
	first, err := srv.sessionNew(nil)
	if err != nil {
		t.Fatal(err)
	}
	second, err := srv.sessionNew(nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	_, _ = srv.sessionDelete(json.RawMessage(`{"sessionId":"` + firstID + `"}`))
	_, _ = srv.sessionDelete(json.RawMessage(`{"sessionId":"` + secondID + `"}`))
}
