...
func TestHeadlessMemoryPromptContextUsesApprovedMemory(t *testing.T) {
	root := t.TempDir()
	...
	item := memory.Item{
		...
		RepoID:     repoID,
		Kind:       "preference",
		Summary:    "Prefer audit-friendly summaries",
		Confidence: "approved",
	}
	...
	got := srv.memoryPromptContext(context.Background(), workdir, "h-1", "write an audit summary")
	if !strings.Contains(got, "[repo/preference mem_headless] Prefer audit-friendly summaries") {
