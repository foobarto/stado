...
// so the per-Linux-architecture blobs below are populated. Dev builds without
...
//
// Per-target embeds live in bundled_linux_<goarch>.go (build tags
...
// this Linux architecture, or ("", binext.ErrNotBundled) when this build has
...
func bundledBinary() (string, error) {
	return binext.Extract(cacheDir(), "rg", bundledBytes, bundledSHA256)
}
